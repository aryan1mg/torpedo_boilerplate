...
class UserManagerTest(asynctest.TestCase):
    ...
    async def test_get_users_raises_exception_with_no_results(self, orm_mock):
        ...
        await self.assertAsyncRaises(BadRequestException, UserManager.get_users({}))

    @patch.object(ORMWrapper, 'get_by_filters')
    async def test_get_users_with_results_return_success(self, orm_mock):
        ...
        result = await UserManager.get_users({})
